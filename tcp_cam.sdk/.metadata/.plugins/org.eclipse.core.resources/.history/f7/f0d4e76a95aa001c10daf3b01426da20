...
u8 ov5640_setparams(u8* data, u16 num){
	u16 t = 0;
	u16 k = 0x0000;
	u8 v = 0x00;
	for(int i=0;i<num;i++){
		...
		k = (data[t]<<8) + (data[t+1]);
		v = data[t+2];
		sccb_write_reg16(k,v);
	}
	return 0;
}

u8 ov5640_init2(u16 cmos_h_pixel, u16 cmos_v_pixel, u16 total_h_pixel, u16 total_v_pixel) {
...
//					sccb_write_reg16(0x3812,0x00); //Timing Voffset[10:8]
			// ISP输入窗口	2500x1900
//			sccb_write_reg16(0x3800,0x00);	// X_ADDR_ST	高8位	66
//			sccb_write_reg16(0x3801,0x42);	// X_ADDR_ST	低8位
//			sccb_write_reg16(0x3802,0x00);	// Y_ADDR_ST	高8位	25
//			sccb_write_reg16(0x3803,0x19);	// Y_ADDR_ST	低8位
//			sccb_write_reg16(0x3804,0x0a);	// X_ADDR_END	高8位	2566
//			sccb_write_reg16(0x3805,0x06);	// X_ADDR_END	低8位
//			sccb_write_reg16(0x3806,0x07);	// Y_ADDR_END	高8位	1925
//			sccb_write_reg16(0x3807,0x85);	// Y_ADDR_END	低8位
			// 预缩放窗口	2500x1500
			sccb_write_reg16(0x3810,0x00);	// Timing Hoffset[11:8]		0
			sccb_write_reg16(0x3811,0x00);	// Timing Hoffset[7:0]
			sccb_write_reg16(0x3812,0x00);	// Timing Voffset[10:8]
			sccb_write_reg16(0x3813,0x57);	// Timing Voffset[7:0]
			// 输出窗口
